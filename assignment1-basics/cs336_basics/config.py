...
class DataConfig:
    ...
    path: str = "data/encoded"
    tokenizer_path: str = "tokenizer/tinystories"
...
class AdamWOptimizerConfig:
    ...
    max_lr: float = 3e-4
    min_lr: float | None = None  # 3e-5
    warmup_iters: int | None = None  # 500
...
class HydraConfig:
    ...
    job_logging: dict[str, Any] = field(
        default_factory=lambda: {
            "handlers": {"file": {"filename": "${hydra.runtime.output_dir}/${hydra.job.name}.log"}},
        },
    )


@dataclass
class MuonOptimizerConfig(AdamWOptimizerConfig):
