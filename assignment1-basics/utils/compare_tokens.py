...
# Forked from https://github.com/donglinkang2021/cs336-assignment1-basics/blob/main/data_utils/compare_tokenizer_demo.py
def compare_tokenizers(tinystories_path, owt_path):
    ...
    print(
        f"TinyStories avg token length: {sum(ts_lengths)/len(ts_lengths):.2f}")
    print(
        f"OpenWebText avg token length: {sum(owt_lengths)/len(owt_lengths):.2f}")
    ...
    union = len(set(ts_vocab.keys()) | set(owt_vocab.keys()))
    jaccard = intersection / union if union > 0 else 0
    print(f"Jaccard Similarity: {jaccard:.4f}")
    
    # 7. Longest Tokens
    ts_longest = sorted(ts_vocab.keys(), key=len, reverse=True)[:5]
    owt_longest = sorted(owt_vocab.keys(), key=len, reverse=True)[:5]
    print(f"\nLongest TinyStories tokens: ")
    for token in ts_longest:
        print(token)
    print(f"\nLongest OpenWebText tokens: ")
    for token in owt_longest:
        print(token)
...
# Usage
compare_tokenizers("model/tinystories", "model/owt")
